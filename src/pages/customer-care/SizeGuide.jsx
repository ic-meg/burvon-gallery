...
const ringSizes = [
  { size: 3, diameter: "1.41", diameterMm: (1.41 * 25.4).toFixed(2), circumferenceIn: "4.43", circumferenceMm: "44.2" },
  { size: 4, diameter: "1.46", diameterMm: (1.46 * 25.4).toFixed(2), circumferenceIn: "4.58", circumferenceMm: "46.1" },
  { size: 5, diameter: "1.57", diameterMm: (1.57 * 25.4).toFixed(2), circumferenceIn: "4.92", circumferenceMm: "49.32" },
  { size: 6, diameter: "1.65", diameterMm: (1.65 * 25.4).toFixed(2), circumferenceIn: "5.16", circumferenceMm: "51.9" },
  { size: 7, diameter: "1.72", diameterMm: (1.72 * 25.4).toFixed(2), circumferenceIn: "5.4", circumferenceMm: "54.4" },
  { size: 8, diameter: "1.81", diameterMm: (1.81 * 25.4).toFixed(2), circumferenceIn: "5.7", circumferenceMm: "57" },
  { size: 9, diameter: "1.89", diameterMm: (1.89 * 25.4).toFixed(2), circumferenceIn: "5.95", circumferenceMm: "59.6" },
];
...
const SizeGuide = () => {
  ...
  const prevFact = () => setFactIndex((prev) => (prev === 0 ? facts.length - 1 : prev - 1));
  const nextFact = () => setFactIndex((prev) => (prev === facts.length - 1 ? 0 : prev + 1));
  ...
  return (
    <Layout>
        ...
        <div className={`absolute z-20 px-4 max-w-xs ${
          ...
          <h1 className="text-[#fff7dc] font-bold bebas text-5xl md:text-9xl whitespace-nowrap">Size Guide</h1>
      ...
      <section className="relative flex flex-col md:flex-row h-auto md:h-[600px] w-screen left-1/2 right-1/2 ml-[-50vw] mr-[-50vw] gap-0 bg-[#1F1F21]">
        ...
        <div className="flex-1 hidden md:flex flex-col justify-center items-center text-center px-6 sm:px-12 lg:px-24" style={{ color: '#fff7dc' }}>
          ...
          <div className="space-y-6 sm:space-y-8 avant w-full max-w-md">
            ...
            <div className="flex items-center">
              <span className="flex justify-center items-center w-12 h-12 text-4xl sm:text-5xl font-bold bebas" style={{ color: '#fff7dc' }}>1</span>
              <span className="text-base sm:text-lg leading-relaxed text-left flex-1">Wrap a strip of paper around your finger.</span>
            </div>
            <div className="flex items-center">
              <span className="flex justify-center items-center w-12 h-12 text-4xl sm:text-5xl font-bold bebas" style={{ color: '#fff7dc' }}>2</span>
              <span className="text-base sm:text-lg leading-relaxed text-left flex-1">Mark the point where the ring closes.</span>
            </div>
            <div className="flex items-center">
              <span className="flex justify-center items-center w-12 h-12 text-4xl sm:text-5xl font-bold bebas" style={{ color: '#fff7dc' }}>3</span>
              <span className="text-base sm:text-lg leading-relaxed text-left flex-1">Measure the length of the paper against the ruler.</span>
            </div>
          </div>
        </div>
      ...
      {!isMobile && (
        <section className="relative w-screen left-1/2 right-1/2 ml-[-50vw] mr-[-50vw] bg-[#323232] py-10 pt-2">
          <div className="max-w-3xl mx-auto flex flex-col items-center mb-4">
            ...
            <img src={BurvonWhite} alt="Burvon Logo" className="w-24 h-auto mb-[-20px]" />
            {/* Title centered below logo */}
            <h2 className="text-4xl bebas font-semibold text-[#fff7dc] text-center tracking-wide mb-1">RING SIZE GUIDE</h2>
          </div>
            ...
            <table className="min-w-full">
              <thead>
                <tr className="bg-[#323232]">
                  <th className="py-3 px-4 text-center text-[#fff7dc] bebas tracking-wide uppercase">Ring <br />Size</th>
                  <th className="py-3 px-4 text-center text-[#fff7dc] bebas tracking-wide uppercase">Diameter<br /> (mm)</th>
                  <th className="py-3 px-4 text-center text-[#fff7dc] bebas tracking-wide uppercase">Diameter<br /> (inch)</th>
                  <th className="py-3 px-4 text-center text-[#fff7dc] bebas tracking-wide uppercase">Circumference<br /> (mm)</th>
                  <th className="py-3 px-4 text-center text-[#fff7dc] bebas tracking-wide uppercase">Circumference<br /> (inch)</th>
                </tr>
              </thead>
              <tbody>
                {ringSizes.map(({ size, diameterMm, diameter, circumferenceMm, circumferenceIn }, index) => (
                  <tr
                    key={index}
                    className={
      ...
      {isMobile ? (
        ...
        <section className="bg-none py-10 px-4 flex flex-col items-center min-h-[300px]">
          <h2 className="text-3xl bebas font-bold text-[#fff7dc] text-left w-full max-w-xs mb-10">DID YOU KNOW?</h2>
...
<div className="flex items-center justify-center gap-4 mt-2">
  <button
    onClick={() => setFactIndex(prev => (prev === 0 ? facts.length - 1 : prev - 1))}
    aria-label="Previous"
    className="p-2"
  >
    <img src={PrevFacts} alt="Previous" className="h-6 w-6" />
      ...
      <button
        onClick={() => setFactIndex(prev => (prev === facts.length - 1 ? 0 : prev + 1))}
        aria-label="Next"
        className="p-2"
      >
