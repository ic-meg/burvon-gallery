      ...
      {order.items.map((item, idx) => (
        <div key={item.order_item_id} className="w-full px-12">
          <div className="flex items-center py-6 border-b border-[#fff7dc]">
            ...
            <div className="w-[120px] pl-35 text-nowrap text-center avant cream-text text-lg">PHP {parseFloat(item.price).toLocaleString(undefined, {minimumFractionDigits:2})}</div>
            ...
            <div className="w-[120px] pl-36 text-nowrap text-center avant cream-text text-lg">
              PHP {(parseFloat(item.price) * item.quantity).toLocaleString(undefined, {minimumFractionDigits:2})}
            </div>
          </div>
      ...
      <div className="flex flex-row px-12 py-8 gap-12 relative">
        ...
        <div className="absolute top-9 right-0 flex flex-col gap-2 avant cream-text text-md min-w-[460px] items-end pr-12">
          <div className="flex justify-between w-full">
            ...
            <span className="avantbold cream-text text-xl">PHP {order.items.reduce((sum, item) => sum + (parseFloat(item.price) * item.quantity), 0).toLocaleString(undefined, {minimumFractionDigits:2})}</span>
          </div>
          <div className="flex justify-between w-full">
            <span className="avantbold cream-text text-xl">Shipping</span>
            <span className="avantbold cream-text text-xl">
              PHP {order.shipping_cost ? parseFloat(order.shipping_cost).toLocaleString(undefined, {minimumFractionDigits:2}) : '0.00'}
            </span>
          </div>
          <div className="w-full border-b border-[#fff7dc] my-2" />
          <div className="flex justify-between w-full avantbold cream-text text-xl">
            <span>Total</span>
            <span>PHP {parseFloat(order.total_price).toLocaleString(undefined, {minimumFractionDigits:2})}</span>
          </div>
        ...
        <div key={item.order_item_id} className="flex gap-3 items-center py-3 border-b border-[#fff7dc]">
          ...
          <div className="flex-1">
            ...
            <div className="flex gap-2 mt-1 avant cream-text text-xs">
              ...
              <span className="w-[70px] text-nowrap">Price: <br/> PHP {parseFloat(item.price).toLocaleString(undefined, {minimumFractionDigits:2})}</span>
              <span className="w-[40px] text-nowrap">Qty: <br/>{item.quantity}</span>
              <span className="w-[40px] text-nowrap">Subtotal: <br/>PHP {(parseFloat(item.price) * item.quantity).toLocaleString(undefined, {minimumFractionDigits:2})}</span>
            </div>
          </div>
        </div>
      ))}
      ...
      <div className="flex flex-col gap-4 py-6">
        ...
        <div className="flex flex-col items-end w-full max-w-[340px] pt-8">
          <div className="w-full flex justify-between items-center mb-4">
              ...
              <span className="avantbold cream-text text-md">PHP {order.items.reduce((sum, item) => sum + (parseFloat(item.price) * item.quantity), 0).toLocaleString(undefined, {minimumFractionDigits:2})}</span>
          </div>
          <div className="w-full flex justify-between items-center mb-4">
              <span className="avantbold cream-text text-md">Shipping</span>
              <span className="avantbold cream-text text-md">
                PHP {order.shipping_cost ? parseFloat(order.shipping_cost).toLocaleString(undefined, {minimumFractionDigits:2}) : ''}
              </span>
          </div>
          <div className="w-full border-b border-[#fff7dc] my-2" />
          <div className="w-full flex justify-between items-center mb-6">
              <span className="avantbold cream-text text-md">Total</span>
              <span className="avantbold cream-text text-md">PHP {parseFloat(order.total_price).toLocaleString(undefined, {minimumFractionDigits:2})}</span>
          </div>
