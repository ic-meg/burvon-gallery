...
// === PRODUCT DATA FOR CLASSICS page. Use your classic items data here! ===
const allClassics = [
  {
    id: 101,
    name: "CLASSIC LYRIC",
...
const heroImages = [
  // You can update these images to classic collection hero banners if you have.
...
// Reuse CustomDropdown unchanged
...
const Classic = () => {
  ...
  const handleImageChange = (cardId, direction) => {
    const card = allClassics.find((item) => item.id === cardId);
    if (!card) return;
  ...
  // Carousel logic for Top Picks Classic Collection
  const [carouselIndex, setCarouselIndex] = useState(0);
  const topPicksClassics = allClassics.slice(0, 8); // or however many you want
  const maxVisible = 4;
  const canPrev = carouselIndex > 0;
  const canNext = carouselIndex < topPicksClassics.length - maxVisible;
  ...
  return (
    <Layout full noPadding>
        ...
        >
          {heroImages.map((image, index) => (
            <picture key={index} className="flex-shrink-0 w-full h-full">
              <img
                src={image.src}
                alt={`Burvon classic collection ${index + 1}`}
      ...
      {/* ===== CLASSIC Section: (Filters only, No big heading) ===== */}
          ...
          >
            ...
            <div>
              ...
              >
                ...
                <img src={showMobileCategory ? DropUpIconBlack : DropDownIconBlack} alt="dropdown" className="ml-2 w-4 h-4" />
              </button>
              {showMobileCategory && (
                <div className="mt-4 space-y-2">
                  {categoryOptions.filter(x => x.value !== "none").map((option) => (
                    <div
                      key={option.value}
                      onClick={() => {
                      ...
                      className={`cursor-pointer avant text-[19px] tracking-wide ${categoryValue === option.value ? "font-bold underline" : ""}`}
                      style={{ color: "#232323" }}
                    >
                      {option.label}
                    </div>
...
<div className="grid grid-cols-2 gap-4 md:hidden items-stretch">
  {allClassics.map((item) => {
    ...
    return (
      ...
      {/* ===== CLASSIC COLLECTION HIGHLIGHTS Section ===== */}
      <section className="relative w-full min-h-[400px] bg-black">
        ...
        <div className="max-w-7xl mx-auto hidden lg:flex flex-col lg:flex-row items-center gap-12 px-6 py-16">
          <div className="flex-1">
            {/* Update this image for classics, if you have */}
            <img
              src={KidsCollHighNeckCrop}
              alt="Classic Collection Highlight"
          ...
          <div className="flex-1 text-[#FFF7DC] pl-8 md:pl-16 lg:pl-24">
            <h2 className="text-2xl lg:text-5xl font-bold bebas tracking-wide mb-4">
              CLASSIC COLLECTION HIGHLIGHTS
            </h2>
            <p className="text-sm md:text-base lg:text-lg text-[#fff7dc] opacity-90 avant leading-snug mb-6">
              Experience timeless elegance with the Burvon Classic Collection.
