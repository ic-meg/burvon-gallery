  ...
  if (categoryData.category_images) {
    try {
      if (Array.isArray(categoryData.category_images)) {
        return categoryData.category_images.map(img => ({ src: img.url || img.src || img }));
      }
      if (typeof categoryData.category_images === 'string') {
        const parsedImages = JSON.parse(categoryData.category_images);
        if (Array.isArray(parsedImages)) {
          return parsedImages.map(img => ({ src: img.url || img.src || img }));
        }
      }
    } catch (error) {
  ...
  if (categoryData.content) {
    try {
      ...
      if (content.category_images && Array.isArray(content.category_images)) {
        return content.category_images.map(img => ({ src: img.url || img.src || img }));
      }
      if (content.heroImages && Array.isArray(content.heroImages)) {
        return content.heroImages.map(img => ({ src: img.url || img.src || img }));
      }
    } catch (error) {
...
const CategoryProducts = () => {
  ...
  const { getCategoryBySlug, fetchCategoryBySlug, hasCategoryData } = useCategory();
  const { productsByCategory, fetchProductsByCategory, loading: productsLoading } = useProduct();
  ...
  const categoryDisplayName = categorySlug ? categorySlug.charAt(0).toUpperCase() + categorySlug.slice(1) : "Products";
  ...
  useEffect(() => {
    ...
    if (!productsByCategory[categorySlug] || productsByCategory[categorySlug].length === 0) {
  ...
  const formatProductData = (product) => {
    ...
    return {
      ...
      oldPrice: product.original_price ? `₱${parseFloat(product.original_price).toFixed(2)}` : 'N/A',
      price: product.current_price ? `₱${parseFloat(product.current_price).toFixed(2)}` : (product.original_price ? `₱${parseFloat(product.original_price).toFixed(2)}` : 'N/A'),
  ...
  const handleImageChange = (cardId, direction) => {
    ...
    setHoveredImageIndexes((prev) => {
      ...
      currentIndex = direction === "next" ? (currentIndex + 1) % len : (currentIndex - 1 + len) % len;
  ...
  return (
    <Layout full noPadding>
      ...
      <section className="relative w-full h-[380px] sm:h-[450px] lg:h-[550px] xl:h-[730px] overflow-hidden bg-black">
        <div className="flex h-full w-full transition-transform duration-700" style={{ transform: `translateX(-${currentIndex * 100}%)` }}>
          {heroImages.map((image, index) => (
            <div key={index} className="flex-shrink-0 w-full h-full bg-[#1F1F21] flex items-center justify-center">
              {image.src && !image.src.includes('PLACEHOLDER') ? (
                <img src={image.src} alt={`Hero ${index + 1}`} className="w-full h-full object-cover" />
              ) : (
                <span className="text-[#FFF7DC] text-2xl avant font-bold">{image.src}</span>
              )}
            </div>
          ))}
        </div>
      ...
      <section className="bg-[#1f1f21] text-[#FFF7DC] pt-8 pb-1 px-4 sm:px-6">
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center justify-between pb-6">
            <h2 className="text-5xl sm:text-6xl bebas tracking-wide whitespace-nowrap">{categoryDisplayName.toUpperCase()}</h2>
            ...
            <div className="hidden sm:flex flex-row flex-wrap items-center gap-8 flex-1 min-w-0">
              <div className="flex flex-col w-56 min-w-[14rem] ml-20">
                <label className="text-xl bebas mb-1 uppercase tracking-wide">Collection Name</label>
                <CustomDropdown
              ...
              <div className="flex flex-col w-50 min-w-[12rem]">
                <label className="text-xl bebas mb-1 uppercase tracking-wide">Sort</label>
                <CustomDropdown
                  options={sortOptions}
                  value={sortValue}
              ...
              <div className="flex flex-col w-full max-w-[200px]">
                <label className="text-xl bebas -mb-1 uppercase tracking-wide">Price</label>
        ...
        {/* Mobile Filter Modal */}
        {mobileFilterOpen && (
          <div className="fixed bottom-0 left-0 w-full z-50 shadow-2xl rounded-t-[30px] p-6 flex flex-col gap-6 animate-slideUp overflow-y-auto"
            style={{ minHeight: "70vh", backgroundColor: "#FFF7DC", color: "#1F1F21", height: "60vh" }}>
            <button className="self-end text-2xl focus:outline-none text-[#232323]" onClick={() => setMobileFilterOpen(false)}>
              &times;
            </button>
            
            <div>
              <button className="w-full flex justify-between items-center cursor-pointer select-none"
                onClick={() => { setShowMobileCollection((prev) => !prev); setShowMobileSort(false); }}>
                <span className="text-2xl bebas uppercase tracking-wide" style={{ textShadow: "0 2px 6px #0001" }}>COLLECTION NAME</span>
              ...
              {showMobileCollection && (
                <div className="mt-4 space-y-2">
                  {collectionOptions.filter(x => x.value !== "none").map((option) => (
                    <div key={option.value} onClick={() => { setCollectionValue(option.value); setShowMobileCollection(false); }}
            ...
            <div>
              <button className="w-full flex justify-between items-center cursor-pointer select-none"
                onClick={() => { setShowMobileSort((prev) => !prev); setShowMobileCollection(false); }}>
                <span className="text-2xl bebas uppercase tracking-wide" style={{ textShadow: "0 2px 6px #0001" }}>SORT</span>
                <img src={showMobileSort ? DropUpIconBlack : DropDownIconBlack} alt="dropdown" className="ml-2 w-4 h-4" />
              </button>
              {showMobileSort && (
                <div className="mt-4 space-y-2">
                  {sortOptions.map((option) => (
                    <div key={option.value} onClick={() => { setSortValue(option.value); setShowMobileSort(false); }}
            ...
            <div>
              ...
              <div className="relative w-full h-8 flex items-center">
                ...
                <div className="absolute top-1/2 h-[3px] bg-[#232] rounded-full -translate-y-1/2"
                  style={{
                    left: `${((minPrice - 300) / (1200 - 300)) * 100}%`,
                    right: `${100 - ((maxPrice - 300) / (1200 - 300)) * 100}%`,
                  }}>
                </div>
                <input type="range" min="300" max="1200" step="50" value={minPrice}
                  onChange={(e) => setMinPrice(Math.min(Number(e.target.value), maxPrice - 50))}
                  className="absolute w-full appearance-none bg-transparent pointer-events-none
                  ...
                  style={{ pointerEvents: "auto" }}
                />
                <input type="range" min="300" max="1200" step="50" value={maxPrice}
                  onChange={(e) => setMaxPrice(Math.max(Number(e.target.value), minPrice + 50))}
                  className="absolute w-full appearance-none bg-transparent pointer-events-none
                    [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:h-4 [&::-webkit-slider-thumb]:w-4
                    [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:bg-[#1F1F21]
            ...
            <button className="-mt-2 w-full bg-[#1F1F21] text-[#FFF7DC] avantbold rounded-md py-2 text-lg tracking-wide"
        ...
        <div className="max-w-7xl mx-auto px-0">
          {productsLoading ? (
            <div className="flex justify-center items-center py-20">
              <div className="text-[#FFF7DC] avant text-lg">Loading products...</div>
            </div>
          ) : formattedProducts.length === 0 ? (
            <div className="flex justify-center items-center py-20">
              <div className="text-[#FFF7DC] avant text-lg">No products available at the moment.</div>
            </div>
          ) : (
            <>
              {/* Mobile Grid */}
              <div className="grid grid-cols-2 gap-4 md:hidden items-stretch">
                {formattedProducts.map((item) => {
                  ...
                  return (
                    <div key={item.id} onClick={() => {}} className="relative bg-[#222] rounded-none overflow-hidden drop-shadow-[0_10px_15px_rgba(0,0,0,1)] cursor-pointer flex flex-col items-center w-full">
                      <div className="relative w-full h-[185px] overflow-hidden bg-black">
                        <div className="absolute top-2 left-2 right-2 flex justify-between items-center z-10">
                          <img src={TryOnIcon} alt="Try On" className="w-4 h-4 cursor-pointer hover:opacity-80" draggable={false} />
                          <img src={AddFavorite} alt="Favorite" className="w-4 h-4 cursor-pointer hover:opacity-80" draggable={false} />
                        </div>
                        <img src={item.images[0]} alt={item.name} className="w-full h-full object-cover"
                          onError={(e) => { e.target.src = 'https://via.placeholder.com/400x400?text=No+Image'; }} />
                      </div>
                      <div className="flex flex-col items-center py-1 px-1" style={{ width: "100%", height: "50px", background: "linear-gradient(90deg, #000000 46%, #666666 100%)" }}>
                        <span className="uppercase text-[#FFF7DC] tracking-widest text-[10px] avantbold leading-tight truncate">{item.name}</span>
                        <span className="text-[10px] tracking-widest text-[#FFF7DC] avant truncate mt-[2px]">{mobileCollection}</span>
                        <div className="flex justify-center items-center gap-1 text-[10px] avantbold mt-1">
                          <span className="line-through text-[#FFF7DC] opacity-50 truncate">{item.oldPrice}</span>
              ...
              <div className="hidden md:grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-10">
                {formattedProducts.map((item) => {
                  ...
                  return (
                    <div key={item.id}
                      onMouseEnter={() => { setHoveredCardId(item.id); setHoveredImageIndexes((prev) => ({ ...prev, [item.id]: 0 })); }}
                      onMouseLeave={() => { setHoveredCardId(null); setHoveredButtonId(null); }}
                      ...
                      style={{ height: isHovered ? "440px" : "375px", transition: "height 0.3s ease, transform 0.3s ease" }}>
                      ...
                      <div className="relative w-full h-[300px] overflow-hidden bg-black">
                        <img src={isHovered ? item.images[currentImageIndex] : item.images[0]} alt={item.name}
                          className="w-full h-full object-cover" onError={(e) => { e.target.src = 'https://via.placeholder.com/400x400?text=No+Image'; }} />
                        {isHovered && item.images.length > 1 && (
                          <>
                            <img onClick={(e) => { e.stopPropagation(); handleImageChange(item.id, "prev"); }} src={PrevIcon} alt="Previous"
                              className="absolute left-4 top-1/2 -translate-y-1/2 w-6 h-6 cursor-pointer hover:opacity-80" draggable={false} />
                            <img onClick={(e) => { e.stopPropagation(); handleImageChange(item.id, "next"); }} src={NextIcon} alt="Next"
                              className="absolute right-4 top-1/2 -translate-y-1/2 w-6 h-6 cursor-pointer hover:opacity-80" draggable={false} />
                          </>
                        )}
                      </div>
                      <div style={{ background: "linear-gradient(90deg, #000000 46%, #666666 100%)" }}
                        ...
                        {isHovered && (
                          <button
                            ...
                            <img src={hoveredButtonId === item.id ? AddBagHover : AddBag} alt="Bag Icon" className="w-4 h-4" />
                            ADD TO BAG
                          </button>
                        )}
                      </div>
      ...
      <section className="relative w-full min-h-[400px] bg-black">
        <div className="max-w-7xl mx-auto hidden lg:flex flex-col lg:flex-row items-center gap-12 px-6 py-16">
          <div className="flex-1">
            {categoryData && categoryData.promo_images && !categoryData.promo_images.includes('PLACEHOLDER') ? (
              <img src={categoryData.promo_images} alt="Promotional" className="w-full h-full object-cover" />
            ) : (
              <span className="text-[#FFF7DC] text-xl avant font-bold">
                {categoryData && categoryData.promo_images ? categoryData.promo_images : 'PROMO IMAGE PLACEHOLDER'}
              </span>
            )}
          </div>
          <div className="flex-1 text-[#FFF7DC] pl-8 md:pl-16 lg:pl-24">
            <h2 className="text-2xl lg:text-5xl font-bold bebas tracking-wide mb-4">
              {categoryData && categoryData.title ? categoryData.title.toUpperCase() : `${categoryDisplayName.toUpperCase()} COLLECTION`}
            </h2>
            <p className="text-sm md:text-base lg:text-lg text-[#fff7dc] opacity-90 avant leading-snug mb-6">
              {categoryData && categoryData.description ? categoryData.description : 'DESCRIPTION PLACEHOLDER'}
            </p>
      ...
      <section className="bg-[#1f1f21] py-14">
        <div className="max-w-7xl mx-auto px-5 relative">
          <div className="flex justify-between items-center pb-8">
            ...
            {!isMobile && (
              <div className="flex space-x-4">
                <div onClick={handlePrev} role="button" tabIndex={0} aria-label="Previous Picks"
                ...
                <div onClick={handleNext} role="button" tabIndex={0} aria-label="Next Picks"
          ...
          {isMobile && (
            <div ref={topScrollRef} className="flex overflow-x-auto overflow-y-hidden scrollbar-hide snap-x snap-mandatory flex-nowrap md:grid md:grid-cols-4 md:gap-5 md:overflow-visible"
              style={{ scrollBehavior: "smooth", WebkitOverflowScrolling: "touch" }}>
              {topPicks.map((item) => (
                <div key={`top-pick-${item.id}`} className="relative bg-[#222] flex-shrink-0 transition-all duration-300 ease-in-out snap-center"
                ...
                return (
                  <div key={`top-pick-${item.id}`}
                    ...
                    <div className="relative w-full h-[300px] flex items-center justify-center overflow-hidden bg-black">
                      <img src={isHovered ? item.images[currentImageIndex] : item.images[0]} alt={item.name}
                        className="object-cover w-full h-full rounded-none transition-all duration-300" draggable={false}
                        onError={(e) => { e.target.src = 'https://via.placeholder.com/400x400?text=No+Image'; }} />
                      {isHovered && item.images.length > 1 && (
                        <>
                          <img onClick={(e) => { e.stopPropagation(); handleImageChange(item.id, "prev"); }} src={PrevIcon} alt="Previous"
