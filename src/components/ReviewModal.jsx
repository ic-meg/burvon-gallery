...
const ReviewModal = ({ open, onClose, products, orderItemId, productId, onSuccess }) => {
  ...
  return (
    <>
      <div className="fixed inset-0 backdrop-blur-sm flex items-center justify-center z-50" onClick={onClose}>
        <div
          className="bg-[#181818] rounded-2xl p-8 min-w-[700px] max-w-[95vw] max-h-[90vh] overflow-y-auto flex flex-col gap-6"
        ...
        >
          <div className="flex items-center justify-between mb-6">
            ...
            <div>
              ...
              <span className="avantbold cream-text ml-2 italic">{ratingLabels[rating - 1]}</span>
            </div>
          </div>
            ...
            <div className="flex flex-col gap-6">
              {products.map((item, idx) => (
                <div key={idx} className="flex gap-4 items-center">
                  ...
                  <div>
                    <div className="avantbold cream-text text-lg leading-tight">{item.name}</div>
            ...
            <div className="flex-1 flex flex-col gap-4">
              ...
              <div className="flex gap-4 flex-wrap">
                ...
                {imagePreviews.map((preview, idx) => (
                  <div key={`img-${idx}`} className="relative w-16 h-16 rounded-lg overflow-hidden border-2 border-[#FFF7DC]">
                ...
                {videoPreviews.map((preview, idx) => (
                  <div key={`vid-${idx}`} className="relative w-16 h-16 rounded-lg overflow-hidden border-2 border-[#FFF7DC]">
                ...
                {imageFiles.length < 5 && (
                  <button
                    className="w-16 h-16 border-2 border-dashed border-[#FFF7DC] rounded-lg flex items-center justify-center cursor-pointer"
              ...
              <div className="flex gap-4 mt-4">
                <button
                  className="avantbold px-8 py-3 rounded-lg text-md border-2 border-[#FFF7DC] cream-text bg-transparent cursor-pointer disabled:opacity-50"
                  onClick={onClose}
                  disabled={loading}
                >
                  CANCEL
                ...
                <button
                  className="avantbold cream-bg metallic-text px-8 py-3 rounded-lg text-md cursor-pointer disabled:opacity-50"
