...
const Necklaces = () => {
  const [necklacesImages, setNecklacesImages] = useState([null, null, null, null, null, null, null, null]);
  ...
  // Handle necklaces carousel image upload
  const handleNecklacesImageUpload = (index, e) => {
    const file = e.target.files[0];
    if (file) {
      const newNecklacesImages = [...necklacesImages];
      newNecklacesImages[index] = file;
      setNecklacesImages(newNecklacesImages);
    }
  };

  // Handle necklaces image removal
  const handleNecklacesImageRemove = (index) => {
    const newNecklacesImages = [...necklacesImages];
    newNecklacesImages[index] = null;
    setNecklacesImages(newNecklacesImages);
  };
  ...
  const handleSaveChanges = () => {
    console.log('Saving necklaces changes...');
    // Add your save logic here
  };

  // Preview handler
  const handlePreview = () => {
    console.log('Opening necklaces preview...');
  ...
  return (
    <div className="grid grid-cols-2 gap-8 -mt-8 h-full">
      ...
      <div className="space-y-8">
        ...
        {/* Necklaces Carousel Section */}
        <div className="space-y-4">
          <h2 className="text-left text-5xl bebas text-black">NECKLACES CAROUSEL</h2>
          <div className="flex items-start space-x-4">
            <div className="grid grid-cols-4 grid-rows-2 gap-2">
              {necklacesImages.map((image, index) => (
                <div key={index} className="relative">
                  ...
                  <label className="absolute inset-0 cursor-pointer">
                    <input 
                      ...
                      className="hidden" 
                      accept="image/*"
                      onChange={(e) => handleNecklacesImageUpload(index, e)}
                    />
                  </label>
                  {/* Remove button for necklaces images */}
                  {image && (
                    <button
                      onClick={() => handleNecklacesImageRemove(index)}
