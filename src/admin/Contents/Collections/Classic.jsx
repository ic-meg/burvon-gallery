...
const Classic = () => {
  const [classicImages, setClassicImages] = useState([null, null, null, null, null, null, null, null]);
  ...
  // Handle classic carousel image upload
  const handleClassicImageUpload = (index, e) => {
    const file = e.target.files[0];
    if (file) {
      const newClassicImages = [...classicImages];
      newClassicImages[index] = file;
      setClassicImages(newClassicImages);
    }
  };

  // Handle classic image removal
  const handleClassicImageRemove = (index) => {
    const newClassicImages = [...classicImages];
    newClassicImages[index] = null;
    setClassicImages(newClassicImages);
  };
  ...
  const handleSaveChanges = () => {
    console.log('Saving classic collection changes...');
    // Add your save logic here
  };

  // Preview handler
  const handlePreview = () => {
    console.log('Opening classic collection preview...');
  ...
  return (
    <div className="grid grid-cols-2 gap-8 -mt-8 h-full">
      ...
      <div className="space-y-8">
        ...
        {/* Classic Collection Carousel Section */}
        <div className="space-y-4">
          <h2 className="text-left text-5xl bebas text-black">CLASSIC COLLECTION CAROUSEL</h2>
                ...
                <div key={index} className="relative">
                  ...
                  <label className="absolute inset-0 cursor-pointer">
                    <input 
                      ...
                      className="hidden" 
                      accept="image/*"
                      onChange={(e) => handleClassicImageUpload(index, e)}
                    />
                  </label>
                  {/* Remove button for classic images */}
                  {image && (
                    <button
                      onClick={() => handleClassicImageRemove(index)}
