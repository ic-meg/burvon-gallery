...
const LoveLanguage = () => {
  const [loveLanguageImages, setLoveLanguageImages] = useState([null, null, null, null, null, null, null, null]);
  ...
  // Handle love language carousel image upload
  const handleLoveLanguageImageUpload = (index, e) => {
    const file = e.target.files[0];
    if (file) {
      const newLoveLanguageImages = [...loveLanguageImages];
      newLoveLanguageImages[index] = file;
      setLoveLanguageImages(newLoveLanguageImages);
    }
  };

  // Handle love language image removal
  const handleLoveLanguageImageRemove = (index) => {
    const newLoveLanguageImages = [...loveLanguageImages];
    newLoveLanguageImages[index] = null;
    setLoveLanguageImages(newLoveLanguageImages);
  };
  ...
  const handleSaveChanges = () => {
    console.log('Saving love language collection changes...');
    // Add your save logic here
  };

  // Preview handler
  const handlePreview = () => {
    console.log('Opening love language collection preview...');
  ...
  return (
    <div className="grid grid-cols-2 gap-8 -mt-8 h-full">
      ...
      <div className="space-y-8">
        ...
        {/* Love Language Collection Carousel Section */}
        <div className="space-y-4">
          <h2 className="text-left text-5xl bebas text-black">LOVE LANGUAGE COLLECTION CAROUSEL</h2>
          <div className="flex items-start space-x-4">
            <div className="grid grid-cols-4 grid-rows-2 gap-2">
              {loveLanguageImages.map((image, index) => (
                <div key={index} className="relative">
                  <div className="w-16 h-16 border-2 border-dashed border-black rounded-lg flex items-center justify-center bg-white">
                    {image ? (
                      <img 
                        ...
                        alt={`Love Language Item ${index + 1}`} 
                  ...
                  <label className="absolute inset-0 cursor-pointer">
                    <input 
                      ...
                      className="hidden" 
                      accept="image/*"
                      onChange={(e) => handleLoveLanguageImageUpload(index, e)}
                    />
                  </label>
                  {/* Remove button for love language images */}
                  {image && (
                    <button
                      onClick={() => handleLoveLanguageImageRemove(index)}
