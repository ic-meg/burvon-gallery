...
const Clash = () => {
  const [clashImages, setClashImages] = useState([null, null, null, null, null, null, null, null]);
  ...
  // Handle clash carousel image upload
  const handleClashImageUpload = (index, e) => {
    const file = e.target.files[0];
    if (file) {
      const newClashImages = [...clashImages];
      newClashImages[index] = file;
      setClashImages(newClashImages);
    }
  };

  // Handle clash image removal
  const handleClashImageRemove = (index) => {
    const newClashImages = [...clashImages];
    newClashImages[index] = null;
    setClashImages(newClashImages);
  };
  ...
  const handleSaveChanges = () => {
    console.log('Saving clash collection changes...');
    // Add your save logic here
  };

  // Preview handler
  const handlePreview = () => {
    console.log('Opening clash collection preview...');
  ...
  return (
    <div className="grid grid-cols-2 gap-8 -mt-8 h-full">
      ...
      <div className="space-y-8">
        ...
        {/* Clash Collection Carousel Section */}
        <div className="space-y-4">
          <h2 className="text-left text-5xl bebas text-black">CLASH COLLECTION CAROUSEL</h2>
          <div className="flex items-start space-x-4">
            <div className="grid grid-cols-4 grid-rows-2 gap-2">
              {clashImages.map((image, index) => (
                <div key={index} className="relative">
                  ...
                  <label className="absolute inset-0 cursor-pointer">
                    <input 
                      ...
                      className="hidden" 
                      accept="image/*"
                      onChange={(e) => handleClashImageUpload(index, e)}
                    />
                  </label>
                  {/* Remove button for clash images */}
                  {image && (
                    <button
                      onClick={() => handleClashImageRemove(index)}
